...
<template>
  <div class="h-full flex flex-col relative bg-theme-surface">
    ...
    <div class="flex-1 flex flex-col min-h-0 overflow-hidden bg-theme-surface">
        ...
        <table class="w-full text-left text-sm whitespace-nowrap">
          <thead class="bg-theme-bg border-b border-theme-border sticky top-0 z-10 shadow-sm">
            <tr class="text-theme-text-sec font-semibold text-[13px]">
              ...
              <th class="px-6 py-3.5 w-[15%]">Stage Name</th>
          ...
          <tbody class="divide-y divide-theme-border">
            ...
            <template v-else>
              <tr v-for="user in users" :key="user.id" class="hover:bg-theme-bg transition-colors group relative">
                <td class="px-6 py-3">
                  <div class="flex items-center gap-3">
                    ...
                    <div v-else class="w-10 h-10 rounded-full bg-slate-100 text-theme-text border border-theme-border flex items-center justify-center font-bold text-sm shadow-sm">
                      {{ user.artist_profile?.stage_name?.charAt(0) || user.name?.charAt(0) }}
                    </div>
                    <div>
                      <div class="font-semibold text-theme-text">{{ user.artist_profile?.stage_name || user.name }}</div>
                      <div class="text-xs text-theme-text-sec font-medium mt-0.5">{{ user.name }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-3 text-theme-text-sec">{{ user.email }}</td>
                <td class="px-6 py-3 text-theme-text-sec">{{ user.artist_profile?.stage_name || '-' }}</td>
      ...
      <div class="bg-theme-surface px-6 py-3 flex items-center justify-between border-t border-theme-border shrink-0">
        ...
        <div class="flex items-center gap-2">
          <button @click="fetchArtists(meta.current_page - 1)" :disabled="meta.current_page === 1" class="px-3 py-1.5 rounded-md border border-theme-border text-theme-text-sec text-[13px] font-medium hover:bg-theme-bg hover:border-slate-300 disabled:opacity-40 disabled:bg-transparent disabled:cursor-not-allowed transition shadow-sm bg-theme-surface">{{ $t('admin.users_page.prev') }}</button>
          <button @click="fetchArtists(meta.current_page + 1)" :disabled="meta.current_page === meta.last_page || meta.last_page === 0" class="px-3 py-1.5 rounded-md border border-theme-border text-theme-text-sec text-[13px] font-medium hover:bg-theme-bg hover:border-slate-300 disabled:opacity-40 disabled:bg-transparent disabled:cursor-not-allowed transition shadow-sm bg-theme-surface">{{ $t('admin.users_page.next') }}</button>
        </div>
      </div>
    </div>
    ...
    <ArtistDrawer 
      ...
      @saved="fetchArtists(meta.current_page); showToast(selectedArtist ? $t('admin.users_page.update_success') : $t('admin.users_page.create_success'))"
    />
  </div>
</template>
