              ...
              <tr v-for="user in users" :key="user.id" class="hover:bg-theme-bg transition-colors group">
                <td class="px-6 py-3">
                  <div class="flex items-center gap-3">
                    ...
                    <div v-else class="w-10 h-10 rounded-full bg-blue-50 dark:bg-blue-500/15 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-500/20 flex items-center justify-center font-bold text-sm shadow-sm">
                ...
                <td class="px-6 py-3">
                  <span v-if="user.is_premium" class="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-bold bg-amber-100 dark:bg-amber-500/15 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20">
                    Premium
                  </span>
                ...
                <td class="px-6 py-3 text-right">
                  <div class="flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity gap-1">
                    <button v-if="user.status === 'Active'" @click="changeStatus(user, 'Suspended')" class="p-1.5 text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-500/15 dark:hover:bg-amber-500/15 rounded-md transition" :title="$t('admin.users_page.action_suspend')">
                      <IconLock size="18" />
                    </button>
                    <button v-else @click="changeStatus(user, 'Active')" class="p-1.5 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/15 dark:hover:bg-emerald-500/15 rounded-md transition" :title="$t('admin.users_page.action_activate')">
                      <IconLockOpen size="18" />
                    </button>
                    <button v-if="user.status !== 'Banned'" @click="changeStatus(user, 'Banned')" class="p-1.5 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/15 dark:hover:bg-rose-500/15 rounded-md transition" :title="$t('admin.users_page.action_ban')">
                      <IconShield size="18" />
                    </button>
                    <!-- Edit -->
                    <button @click="openEdit(user)" class="p-1.5 text-admin-primary dark:text-blue-400 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-500/15 dark:hover:bg-blue-500/15 rounded-md transition ml-1" :title="$t('admin.users_page.action_edit')">
                      <IconEdit size="18" />
                    </button>
                    <!-- Context menu -->
                    <button @click="deleteUser(user)" class="p-1.5 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/15 dark:hover:bg-rose-500/15 rounded-md transition" :title="$t('admin.users_page.action_delete')">
                      <IconTrash size="18" />
                    </button>
                  </div>
                </td>
      ...
      <div class="bg-theme-surface px-6 py-3 flex items-center justify-between border-t border-theme-border shrink-0">
        ...
        <div class="flex items-center gap-2">
          <button 
            ...
            class="px-3 py-1.5 rounded-md border border-theme-border text-theme-text-sec text-[13px] font-medium hover:bg-theme-bg hover:border-slate-300 disabled:opacity-40 disabled:bg-transparent disabled:cursor-not-allowed transition shadow-sm bg-theme-surface"
          >
