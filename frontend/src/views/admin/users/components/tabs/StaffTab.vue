...
const deleteUser = async (user: any) => {
  if (!confirm(`Bạn có chắc chắn muốn xóa nhân viên ${user.name}? Hành động này không thể hoàn tác.`)) return;
  try {
...
const changeStatus = async (user: any, newStatus: string) => {
  if (!confirm(`Xác nhận chuyển trạng thái nhân viên này thành: ${newStatus}?`)) return;
...
<template>
  <div class="h-full flex flex-col relative bg-white">
    ...
    <div class="flex-1 flex flex-col min-h-0 overflow-hidden bg-white">
        ...
        <table class="w-full text-left text-sm whitespace-nowrap">
          <thead class="bg-slate-50 border-b border-slate-200 sticky top-0 z-10 shadow-sm">
            <tr class="text-slate-500 font-semibold text-[13px]">
              <th class="px-6 py-3.5 w-[30%]">Staff Member</th>
              <th class="px-6 py-3.5 w-[25%]">Email</th>
              <th class="px-6 py-3.5 w-[20%]">Role / Permissions</th>
              <th class="px-6 py-3.5 w-[10%]">Status</th>
          ...
          <tbody class="divide-y divide-slate-100">
            ...
            <tr v-else-if="users.length === 0">
              <td colspan="5" class="p-0">
                <EmptyState title="No staff found" description="Invite your team members to help manage the platform." :icon="IconShieldLock">
                  <template #action>
              ...
              <tr v-for="user in users" :key="user.id" class="hover:bg-slate-50 transition-colors group">
                <td class="px-6 py-3">
                  <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-slate-800 text-white flex items-center justify-center font-bold text-sm shadow-sm ring-2 ring-slate-100">
                      {{ user.name?.charAt(0) || 'S' }}
                    </div>
                    <div>
                      <div class="font-semibold text-slate-900">{{ user.name }}</div>
                      <div class="text-xs text-slate-500 font-medium mt-0.5">Joined {{ formatDate(user.created_at) }}</div>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-3 text-slate-600 font-medium">{{ user.email }}</td>
                <td class="px-6 py-3">
                  <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[12px] font-semibold bg-slate-100 text-slate-700 border border-slate-200/60" v-if="user.roles && user.roles.length">
                    <IconShieldLock size="14" class="mr-1.5 text-slate-500" />
                    {{ user.roles.map((r: any) => r.name).join(', ') }}
                  </span>
                  <span v-else class="text-slate-400 italic text-xs">No specific roles</span>
                </td>
                ...
                <td class="px-6 py-3 text-right">
                  <div class="flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity gap-1">
                    ...
                    <button @click="openEdit(user)" class="p-1.5 text-admin-primary hover:bg-blue-50 rounded-md transition ml-1" title="Sửa quyền"><IconEdit size="18" /></button>
                    <button @click="deleteUser(user)" class="p-1.5 text-rose-600 hover:bg-rose-50 rounded-md transition" title="Xóa nhân sự"><IconTrash size="18" /></button>
                  </div>
                </td>
              </tr>
            </template>
      ...
      <div class="bg-white px-6 py-3 flex items-center justify-between border-t border-slate-200 shrink-0">
        ...
        <div class="flex items-center gap-2">
          <button @click="fetchStaff(meta.current_page - 1)" :disabled="meta.current_page === 1" class="px-3 py-1.5 rounded-md border border-slate-200 text-slate-600 text-[13px] font-medium hover:bg-slate-50 disabled:opacity-40 shadow-sm bg-white">Previous</button>
          <button @click="fetchStaff(meta.current_page + 1)" :disabled="meta.current_page === meta.last_page || meta.last_page === 0" class="px-3 py-1.5 rounded-md border border-slate-200 text-slate-600 text-[13px] font-medium hover:bg-slate-50 disabled:opacity-40 shadow-sm bg-white">Next</button>
        </div>
      </div>
    </div>
    ...
    <StaffDrawer 
      ...
      @saved="fetchStaff(meta.current_page); showToast(selectedStaff ? 'Cập nhật thành công!' : 'Tạo mới thành công!')"
    />
  </div>
</template>
