...
import StaffDrawer from '@/components/admin/features/users/drawers/StaffDrawer.vue';
...
const fetchStaff = async (page = 1) => {
  ...
  try {
    const res = await api.get('/admin/users', {
      params: { 'filter[role]': 'admin', search: searchQuery.value, status: statusFilter.value, page }
    });
...
const handleSearch = () => fetchStaff(1);
...
const deleteUser = async (user: any) => {
  ...
  try {
    ...
    fetchStaff(meta.value.current_page);
  } catch (e: any) {
...
const getStatusBadgeClass = (status: string) => {
  switch (status?.toLowerCase()) {
    case 'active': return 'bg-emerald-50 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border-emerald-200/60 dark:border-emerald-500/20';
    case 'suspended': return 'bg-amber-50 dark:bg-amber-500/15 text-amber-700 dark:text-amber-400 border-amber-200/60 dark:border-amber-500/20';
    case 'banned': return 'bg-rose-50 dark:bg-rose-500/15 text-rose-700 dark:text-rose-400 border-rose-200/60 dark:border-rose-500/20';
...
<template>
  <div class="h-full flex flex-col relative bg-theme-surface">
      ...
      <div class="flex items-center gap-3 flex-1">
        <div class="relative w-full max-w-[340px] group">
          ...
          <input 
            ...
            type="text" 
            class="w-full h-9 pl-9 pr-3 border border-slate-300 rounded-[8px] text-sm focus:outline-none focus:ring-1 focus:ring-admin-primary focus:border-admin-primary transition-colors shadow-sm bg-theme-surface" 
        ...
        <div class="relative group">
          ...
          <select 
            ...
            @change="handleSearch"
            class="h-9 border border-slate-300 rounded-[8px] pl-9 pr-8 text-sm focus:outline-none focus:ring-1 focus:ring-admin-primary focus:border-admin-primary text-theme-text bg-theme-surface transition-colors shadow-sm font-medium appearance-none cursor-pointer"
          >
    ...
    <div class="flex-1 flex flex-col min-h-0 overflow-hidden bg-theme-surface">
        ...
        <table class="w-full text-left text-sm whitespace-nowrap">
          <thead class="bg-theme-bg border-b border-theme-border sticky top-0 z-10 shadow-sm">
            <tr class="text-theme-text-sec font-semibold text-[13px]">
              ...
              <th class="px-6 py-3.5 w-[15%]">Role</th>
          ...
          <tbody class="divide-y divide-theme-border">
            ...
            <tr v-else-if="users.length === 0">
              <td colspan="6" class="p-0">
                ...
                >
                  <template #action>
                    <BaseAdminButton variant="secondary" :icon="IconPlus" @click="openCreate">Invite First Staff</BaseAdminButton>
                  </template>
                </EmptyState>
              </td>
            </tr>
              ...
              <tr v-for="user in users" :key="user.id" class="hover:bg-theme-bg transition-colors group">
                <td class="px-6 py-3">
                  <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-slate-800 dark:bg-slate-700 text-white flex items-center justify-center font-bold text-sm shadow-sm ring-2 ring-theme-bg">
                ...
                <td class="px-6 py-3">
                  <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[12px] font-semibold bg-theme-bg text-theme-text border border-theme-border" v-if="user.roles && user.roles.length">
                    <IconShieldLock size="14" class="mr-1.5 text-theme-text-sec" />
                    {{ user.roles.map((r: any) => r.name).join(', ') }}
                  </span>
                  <span v-else class="text-theme-text-sec italic text-xs">No specific roles</span>
                </td>
                <td class="px-6 py-3"><span :class="['px-2.5 py-1 text-[11px] font-bold rounded-full border tracking-wide uppercase', getStatusBadgeClass(user.status)]">{{ formatStatus(user.status) }}</span></td>
                ...
                <td class="px-6 py-3 text-right">
                  <div class="flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity gap-1">
                    <button v-if="user.status === 'Active'" @click="changeStatus(user, 'Suspended')" class="p-1.5 text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-500/15 rounded-md transition" :title="$t('admin.users_page.action_suspend')"><IconLock size="18" /></button>
                    <button v-else @click="changeStatus(user, 'Active')" class="p-1.5 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/15 rounded-md transition" :title="$t('admin.users_page.action_activate')"><IconLockOpen size="18" /></button>
                    <button v-if="user.status !== 'Banned'" @click="changeStatus(user, 'Banned')" class="p-1.5 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/15 rounded-md transition" :title="$t('admin.users_page.action_ban')"><IconShield size="18" /></button>
                    
                    <button @click="openEdit(user)" class="p-1.5 text-admin-primary dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-500/15 rounded-md transition ml-1" :title="$t('admin.users_page.action_edit')"><IconEdit size="18" /></button>
                    <button @click="deleteUser(user)" class="p-1.5 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/15 rounded-md transition" :title="$t('admin.users_page.action_delete')"><IconTrash size="18" /></button>
                  </div>
                </td>
              </tr>
            </template>
      ...
      <div class="bg-theme-surface px-6 py-3 flex items-center justify-between border-t border-theme-border shrink-0">
        ...
        <div class="flex items-center gap-2">
          <button @click="fetchStaff(meta.current_page - 1)" :disabled="meta.current_page === 1" class="px-3 py-1.5 rounded-md border border-theme-border text-theme-text-sec text-[13px] font-medium hover:bg-theme-bg hover:border-slate-300 disabled:opacity-40 disabled:bg-transparent disabled:cursor-not-allowed transition shadow-sm bg-theme-surface">
            {{ $t('admin.users_page.prev') }}
          </button>
          <button @click="fetchStaff(meta.current_page + 1)" :disabled="meta.current_page === meta.last_page || meta.last_page === 0" class="px-3 py-1.5 rounded-md border border-theme-border text-theme-text-sec text-[13px] font-medium hover:bg-theme-bg hover:border-slate-300 disabled:opacity-40 disabled:bg-transparent disabled:cursor-not-allowed transition shadow-sm bg-theme-surface">
    ...
    <StaffDrawer 
      ...
      @saved="fetchStaff(meta.current_page); showToast(selectedStaff ? t('admin.users_page.update_success') : t('admin.users_page.create_success'))"
    />
  </div>
</template>
