...
import { IconLock, IconKey, IconPlayerPlay, IconCheck } from '@tabler/icons-vue';
...
onMounted(() => {
  ...
  if (!email.value || !token.value) {
    error.value = 'Đường dẫn khôi phục không hợp lệ hoặc thiếu thông tin.';
  }
});
...
const handleResetPassword = async () => {
  if (!email.value || !token.value) {
    error.value = 'Thiếu thông tin khôi phục từ URL.';
    return;
  }
  
  if (!validatePassword(password.value)) {
    error.value = 'Mật khẩu phải từ 8 ký tự, gồm chữ hoa, chữ thường, số và ký tự đặc biệt.';
    return;
  }
  ...
  try {
    await api.post('/guest/auth/password/reset', {
      email: email.value,
      token: token.value,
    ...
    error.value = err.response?.data?.message || 'Khôi phục thất bại. Đường dẫn có thể đã hết hạn.';
  } finally {
    loading.value = false;
  }
};
