...
<template>
  <button
    ...
    :class="[
      ...
      // Variant definitions
      variant === 'secondary' ? 'bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 focus:ring-slate-200' :
      variant === 'danger' ? 'bg-rose-600 text-white hover:bg-rose-700 focus:ring-rose-500/30 border border-transparent' :
      variant === 'ghost' ? 'bg-transparent text-slate-600 hover:bg-slate-100 hover:text-slate-900 focus:ring-slate-200 shadow-none' :
