...
<template>
 ...
 <div>
 <label class="block text-sm font-medium text-theme-text-sec mb-2 flex justify-between">
 <span>Ảnh Banner</span>
 <span class="text-xs text-theme-text-sec font-normal">{{ $t('admin.banners.image_req') }}</span>
 </label>
 
 <div 
 v-if="!imagePreview"
 class="aspect-[3/1] bg-theme-surface-hover border-2 border-dashed border-theme-border rounded-xl flex flex-col items-center justify-center cursor-pointer hover:bg-theme-surface-hover hover:border-theme-primary/50 transition-colors"
 @click="() => { const el = $refs.imageInput as HTMLInputElement; el.click() }"
 >
 <IconPhoto size="32" class="text-theme-text-sec mb-2" />
 <span class="text-sm text-theme-text-sec font-medium">Nhấn để tải ảnh lên</span>
 </div>
 <div v-else class="aspect-[3/1] rounded-xl overflow-hidden relative group border border-theme-border">
 ...
 <div class="flex items-center gap-3 p-4 border border-theme-border rounded-xl bg-theme-surface-hover">
 <div class="flex-1">
 <h4 class="text-sm font-bold text-theme-text">Trạng thái hiển thị</h4>
 <p class="text-xs text-theme-text-sec">Banner sẽ lập tức xuất hiện trên trang chủ nếu được bật.</p>
 </div>
 <button 
 type="button" 
 ...
 :class="form.is_active ? 'bg-theme-success' : 'bg-theme-border'"
 @click="form.is_active = !form.is_active"
 >
 <span class="pointer-events-none inline-block h-6 w-6 transform rounded-full bg-theme-surface shadow ring-0 transition duration-200 ease-in-out" :class="form.is_active ? 'translate-x-5' : 'translate-x-0'"></span>
 </button>
 </div>

 <!-- Error Message -->
 <div v-if="bannerStore.error" class="p-3 bg-theme-danger/10 text-theme-danger text-sm rounded-lg">
 {{ bannerStore.error }}
 </div>
