...
<template>
 ...
 <!-- Upload Zone -->
 <div>
 <label class="block text-sm font-medium text-theme-text-sec mb-2">File Âm thanh</label>
 
 <div 
 v-if="!selectedFile"
 class="border-2 border-dashed rounded-xl p-8 text-center transition-colors cursor-pointer"
 :class="isDragging ? 'border-theme-primary bg-theme-primary/5' : 'border-theme-border hover:bg-theme-surface-hover '"
 @dragover.prevent="isDragging = true"
 @dragleave.prevent="isDragging = false"
 @drop.prevent="handleDrop"
 @click="fileInput?.click()"
 >
 <div class="w-12 h-12 bg-theme-surface-hover text-theme-text-sec rounded-full flex items-center justify-center mx-auto mb-3">
 <IconUpload size="24" />
 </div>
 ...
 <!-- Selected File View -->
 <div v-else class="p-4 bg-theme-surface-hover rounded-xl border border-theme-border flex items-center gap-4">
 <div class="w-12 h-12 bg-theme-primary/10 text-theme-primary rounded-lg flex items-center justify-center shrink-0">
 <IconMusic size="24" />
 </div>
 <div class="flex-1 min-w-0">
 <p class="text-sm font-medium text-theme-text truncate">{{ selectedFile.name }}</p>
 <p class="text-xs text-theme-text-sec">{{ (selectedFile.size / (1024 * 1024)).toFixed(2) }} MB</p>
 </div>
 <button 
 v-if="!inventoryStore.isUploading" 
 type="button" 
 @click="removeFile" 
 class="p-2 text-theme-text-sec hover:text-theme-danger hover:bg-theme-danger/10 rounded-lg transition-colors shrink-0"
 >
 <IconX size="18" />
 </button>
 </div>
 ...
 <span class="text-theme-primary font-medium">{{ inventoryStore.uploadStatusText }}</span>
 <span class="text-theme-text-sec">{{ inventoryStore.uploadProgress }}%</span>
 </div>
 <div class="w-full bg-theme-surface-hover rounded-full h-2 overflow-hidden">
 <div 
 ...
 <label class="block text-sm font-medium text-theme-text-sec mb-1">Nghệ Sĩ Thể Hiện</label>
 
 <div v-if="selectedArtistName" class="flex items-center gap-2 p-2.5 border border-theme-border rounded-lg bg-theme-surface-hover ">
 <div class="w-6 h-6 rounded-full bg-theme-primary/15 text-theme-primary flex items-center justify-center text-xs font-bold">
 {{ selectedArtistName.charAt(0).toUpperCase() }}
 </div>
 <span class="text-sm font-medium flex-1 text-theme-text ">{{ selectedArtistName }}</span>
 <button v-if="!fixedArtistId" type="button" @click="clearArtist" class="text-theme-text-sec hover:text-theme-text-sec p-1">
 <IconX size="16" />
 </button>
 </div>
 ...
 class="w-full pl-9 pr-4 py-2.5 bg-theme-surface ] border border-theme-border text-sm rounded-lg focus:ring-2 focus:ring-admin-primary focus:border-theme-primary outline-none transition-all "
 ...
 <!-- Dropdown -->
 <div v-if="showArtistDropdown && artistSearchQuery" class="absolute z-10 w-full mt-1 bg-theme-surface border border-theme-border rounded-lg shadow-lg overflow-hidden max-h-60 overflow-y-auto">
 <div v-if="isSearchingArtist" class="p-3 text-center text-sm text-theme-text-sec">Đang tìm kiếm...</div>
 <div v-else-if="artists.length === 0" class="p-3 text-center text-sm text-theme-text-sec">Không tìm thấy nghệ sĩ nào</div>
 <button
 v-else
 v-for="artist in artists"
 :key="artist.id"
 type="button"
 class="w-full text-left px-4 py-2 text-sm hover:bg-theme-surface-hover transition-colors flex items-center gap-3"
 @click="selectArtist(artist)"
 >
 <div class="w-8 h-8 rounded-full bg-theme-primary/15 text-theme-primary flex items-center justify-center font-bold text-xs">
 {{ (artist.artistProfile?.stage_name || artist.name).charAt(0).toUpperCase() }}
 </div>
 <div>
 <div class="font-medium text-theme-text ">{{ artist.artistProfile?.stage_name || artist.name }}</div>
 <div class="text-xs text-theme-text-sec">{{ artist.email }}</div>
 </div>
 </button>
 </div>
 </div>
 ...
 <!-- Genre Select -->
 <div>
 <label class="block text-sm font-medium text-theme-text-sec mb-1">Thể Loại Nhạc</label>
 <select 
 v-model="form.genre_id"
 required
 class="w-full bg-theme-surface ] border border-theme-border text-theme-text text-sm rounded-lg focus:ring-2 focus:ring-admin-primary focus:border-theme-primary block p-2.5 outline-none transition-colors"
 >
 ...
 <!-- Error Message -->
 <div v-if="inventoryStore.error" class="p-3 bg-theme-danger/10 text-theme-danger text-sm rounded-lg">
