  ...
  <form @submit.prevent="handleSubmit" class="space-y-6">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="col-span-1">
        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Ảnh bìa Album</label>
        <div 
          v-if="!coverImagePreview"
          class="aspect-square bg-slate-50 dark:bg-[#1E232D] border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
          @click="() => { const el = $refs.imageInput as HTMLInputElement; el.click() }"
        >
          <IconPhoto size="32" class="text-slate-400 mb-2" />
          <span class="text-sm text-slate-500 font-medium">Tải ảnh lên (Tỉ lệ 1:1)</span>
        </div>
        <div v-else class="aspect-square rounded-xl overflow-hidden relative group border border-slate-200">
      ...
      <div class="col-span-1 md:col-span-2 space-y-4">
        ...
        <div class="relative artist-search-container">
          <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nghệ Sĩ</label>
          <div v-if="selectedArtistName" class="flex items-center gap-2 p-2 border border-slate-200 rounded-lg bg-slate-50">
            <span class="text-sm font-medium flex-1 pl-2">{{ selectedArtistName }}</span>
            <button type="button" @click="clearArtist" class="p-1 hover:text-rose-500 text-slate-400">
              <IconX size="16" />
            </button>
          </div>
          <div v-else class="relative">
            ...
            />
            <IconSearch size="16" class="absolute left-3 top-2.5 text-slate-400" />
            
            <!-- Dropdown -->
            <div v-if="showArtistDropdown && artistSearchQuery" class="absolute z-10 w-full mt-1 bg-white border shadow-lg rounded-lg max-h-48 overflow-y-auto">
              <button v-for="artist in artists" :key="artist.id" type="button" class="w-full text-left px-4 py-2 hover:bg-slate-50 text-sm" @click="selectArtist(artist)">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Loại Phát Hành</label>
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Ngày Phát Hành</label>
    ...
    <div class="border-t border-slate-200 pt-6 mt-6">
      <h3 class="text-sm font-bold text-slate-700 mb-4 uppercase">Gán Bài Hát Vào Album</h3>
      
      <div v-if="!form.artist_id" class="p-4 bg-amber-50 text-amber-700 text-sm rounded-lg border border-amber-200 text-center">
        {{ $t('admin.albums.select_artist_first') }}
      </div>
      
      <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <!-- Left: Unassigned -->
        <div class="border border-slate-200 rounded-xl overflow-hidden flex flex-col h-64 bg-slate-50">
          <div class="bg-slate-100 border-b px-3 py-2 text-xs font-bold text-slate-600 uppercase flex justify-between items-center">
          ...
          <div class="p-2 flex-1 overflow-y-auto space-y-1">
            <div v-if="albumStore.isFetchingSongs" class="text-center text-xs p-4 text-slate-400">Đang tải...</div>
            <div v-else-if="availableSongs.length === 0" class="text-center text-xs p-4 text-slate-400">Không có bài hát nào</div>
            
            <div v-for="song in availableSongs" :key="song.id" class="group flex items-center justify-between p-2 bg-white rounded border border-slate-100 hover:border-admin-primary/50 cursor-pointer shadow-sm transition-all" @click="moveToRight(song)">
              <div class="flex items-center gap-2 truncate">
                <IconMusic size="14" class="text-slate-400" />
                <span class="text-sm text-slate-700 truncate">{{ song.title }}</span>
              </div>
              <IconChevronRight size="16" class="text-slate-300 group-hover:text-admin-primary" />
            </div>
          </div>
        </div>

        <!-- Right: Assigned -->
        <div class="border border-admin-primary/30 rounded-xl overflow-hidden flex flex-col h-64 bg-blue-50/30">
          <div class="bg-admin-primary/10 border-b border-admin-primary/20 px-3 py-2 text-xs font-bold text-admin-primary uppercase flex justify-between items-center">
            {{ $t('admin.albums.album_songs') }}
            <span class="bg-admin-primary text-white px-2 rounded-full">{{ selectedSongs.length }}</span>
          </div>
          <div class="p-2 flex-1 overflow-y-auto space-y-1">
            <div v-if="selectedSongs.length === 0" class="text-center text-xs p-4 text-admin-primary/50">Chưa chọn bài hát nào</div>
            
            <div v-for="(song, idx) in selectedSongs" :key="song.id" class="group flex items-center gap-2 p-2 bg-white rounded border border-admin-primary/20 hover:border-rose-300 cursor-pointer shadow-sm transition-all" @click="moveToLeft(song)">
              <div class="text-xs font-bold text-slate-400 w-4 text-center">{{ idx + 1 }}</div>
              <div class="flex-1 truncate text-sm text-slate-700">{{ song.title }}</div>
              <IconChevronLeft size="16" class="text-slate-300 group-hover:text-rose-500" />
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Error Message -->
    <div v-if="albumStore.error" class="p-3 bg-rose-50 text-rose-700 text-sm rounded-lg">
      {{ albumStore.error }}
    </div>
