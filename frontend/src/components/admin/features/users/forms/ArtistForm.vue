...
<template>
  <form id="artist-form" @submit.prevent="validateAndSubmit" class="space-y-3">
    ...
    <BaseAdminInput v-model="formData.name" label="Tên thật" placeholder="Ví dụ: Nguyễn Văn A" required :disabled="loading" />
    <BaseAdminInput v-model="formData.stage_name" label="Nghệ danh" placeholder="Ví dụ: Sơn Tùng M-TP" required :disabled="loading" />
    <BaseAdminInput v-model="formData.email" type="email" label="Địa chỉ Email" placeholder="artist@domain.com" required :disabled="loading" />
    
    <template v-if="!initialData">
      <BaseAdminInput v-model="formData.password" type="password" label="Mật khẩu" placeholder="Tối thiểu 8 ký tự, có ký tự đặc biệt" required :disabled="loading" />
      <BaseAdminInput v-model="formData.password_confirmation" type="password" label="Xác nhận mật khẩu" placeholder="Nhập lại mật khẩu để xác minh" required :disabled="loading" />
    </template>
  </form>
</template>
