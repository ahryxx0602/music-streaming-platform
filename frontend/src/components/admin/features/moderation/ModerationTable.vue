...
<template>
  <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
      ...
      <table class="w-full text-left border-collapse min-w-[800px]">
        <thead>
          <tr class="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
            ...
            <th class="px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Ngày Tải Lên</th>
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <tr v-else-if="moderationStore.pendingSongs.length === 0">
            <td colspan="5" class="px-4 py-16 text-center">
              ...
              <h3 class="text-sm font-bold text-slate-700 mb-1">Không có bài hát nào chờ duyệt</h3>
              <p class="text-xs text-slate-500">Tất cả các bài hát đã được xử lý.</p>
            </td>
          </tr>
          
          <tr v-for="song in moderationStore.pendingSongs" :key="song.id" class="hover:bg-slate-50 transition-colors group">
            <td class="px-4 py-3 text-sm text-slate-500">#{{ song.id }}</td>
            <td class="px-4 py-3">
              <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-md bg-indigo-50 text-indigo-400 flex items-center justify-center shrink-0">
                  <IconMusic size="20" />
                </div>
                <div>
                  <div class="font-medium text-slate-900 line-clamp-1">{{ song.title }}</div>
                  <div class="text-[11px] text-slate-500 mt-0.5">{{ song.genre?.name || '---' }}</div>
                </div>
              </div>
            </td>
            <td class="px-4 py-3">
              <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-medium text-slate-700">{{ song.artist?.artistProfile?.stage_name || song.artist?.name || 'Unknown' }}</span>
              </div>
            </td>
            <td class="px-4 py-3">
              <span class="text-sm text-slate-500">{{ formatDate(song.created_at) }}</span>
            </td>
            <td class="px-4 py-3 text-right">
              <button 
                @click="emit('preview', song)" 
                class="px-3 py-1.5 bg-admin-primary/10 text-admin-primary hover:bg-admin-primary hover:text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-1.5 ml-auto"
              >
                <IconEye size="16" />
                Kiểm duyệt
              </button>
    ...
    <div class="p-3 border-t border-slate-200 flex items-center justify-between text-sm text-slate-500 bg-slate-50 shrink-0">
      <div>Hiển thị <span class="font-bold text-slate-900">{{ moderationStore.pendingSongs.length }}</span> bài hát</div>
