...
| Controller | Tiếp nhận dữ liệu, gọi Action điều phối |
| Action | Thực thi nghiệp vụ đơn lẻ (Single Responsibility) |
| Repository | Truy vấn cơ sở dữ liệu thông qua Model |
| Eloquent ORM | Mapping Model ↔ Database |
| MySQL | Lưu trữ dữ liệu gốc |
| API Resource | Format chuẩn hóa dữ liệu trả về cho Client |
...
### Cấu trúc thư mục (nwidart/laravel-modules)

```text
/
├── app/ (Base/Global Logic)
└── Modules/
    ├── Authentication/
    │   ├── Actions/             # Business Logic độc lập
    │   ├── Http/
    │   │   ├── Controllers/     # Điều hướng request
    │   │   ├── Requests/        # Validate input
    │   │   └── Resources/       # Format JSON output
    │   ├── Models/              # Eloquent
    │   ├── Repositories/        # Xử lý query DB
...
- Business Logic nằm trong tầng **Action** (Mỗi class 1 action duy nhất theo Single Responsibility).
- **Repository** chỉ thực hiện truy vấn dữ liệu thông qua Model.
- Không truy cập trực tiếp Repository của Module khác.
- Controller nhận kết quả từ Action và bọc qua **Resource** trước khi trả về.
- Giao tiếp chéo giữa các Module thông qua Action hoặc Event.
- Tuân thủ nghiêm ngặt nguyên tắc SOLID.
