...
- **Localization:** `vue-i18n` (Xử lý Đa ngôn ngữ)
- **Styling & Theming:** CSS Variables (Vanilla CSS) kết hợp Design System. Floating UI cho Dropdown/Tooltip.
- **Icons:** Thư viện `@tabler/icons-vue` (Icon system đồng nhất, SVG nhẹ).
- **Animations & Effects:** Motion (Vue), CSS Transitions, GSAP (dành cho Visualizer phức tạp), tsParticles (Nền động).
- **Core Utilities:** VueUse (Hooks chuẩn cho LocalStorage, DarkMode, MediaControls).
- **Advanced UI:** `vue-virtual-scroller` (Tối ưu danh sách dài), `@formkit/drag-and-drop` (Kéo thả mượt mà).
- **Audio Engine:** `Howler.js` (Xử lý âm thanh đa nền tảng, Crossfade, Volume control).
...
├── locales/           # File dịch đa ngôn ngữ (vi.json, en.json)
...
Dự án áp dụng chặt chẽ bộ **Design System (Theme: Aurora Blue)** được quy định tại `[09-design-system.md]`. Để duy trì sự nhất quán cao cấp này, hệ thống kết hợp TailwindCSS v4 và Vanilla CSS theo nguyên tắc sau:

1. **CSS Variables & Theming (Giao diện Sáng/Tối):**
   - Áp dụng toàn bộ hệ thống mã HEX trong Design System thành các biến CSS (`--color-primary`, `--bg-main`, `--text-primary`) bên trong file `src/assets/main.css`.
   - **Cơ chế Light/Dark:** Khai báo 2 bộ biến: `:root` (cho Light Mode) và `.dark` (cho Dark Mode).
   - Quản lý trạng thái chuyển đổi Theme bằng plugin `@vueuse/core` (`useDark`, `useToggle`). Khi người dùng đổi nền sáng/tối, class `dark` sẽ tự động add/remove ở thẻ `<html>`, màu sắc toàn bộ app sẽ biến đổi lập tức. Không sử dụng class prefix `dark:` của Tailwind rải rác khắp template để tránh code bị phình to.
...
Tài liệu tham chiếu cốt lõi: `[09-design-system.md]`

1. **Theme Sáng/Tối:** Hỗ trợ mượt mà cả 2 chế độ, nhưng "Aurora Blue" (Dark mode) vẫn là điểm nhấn sang trọng nhất.
2. **Glassmorphism:** Sử dụng hiệu ứng mờ kính `backdrop-filter: blur(16px)` cho Navbar và Sticky Header (cả 2 mode sáng và tối).
3. **Typography:** Font chữ hiện đại (Inter, Outfit).
4. **Skeleton Loading:** Hiển thị Skeleton màu `#131B2F` thay vì Spinner giật cục khi load dữ liệu.
5. **Micro-animations:** Hover effects nâng Card `translateY(-4px)`, thanh tiến trình chạy `linear`, hiệu ứng đĩa than xoay vô tận.
6. **Icon System:** BẮT BUỘC sử dụng đồng nhất thư viện `@tabler/icons-vue` cho toàn bộ biểu tượng trong hệ thống. Tuyệt đối không dùng SVG thô (raw) để tránh rối code và khó chỉnh màu, trừ khi là logo độc quyền.

---

## 8. Đa ngôn ngữ (Localization - i18n)

Frontend hỗ trợ kiến trúc đa ngôn ngữ để mở rộng người dùng:
1. **Thư viện Core:** Sử dụng plugin `vue-i18n`.
2. **Cấu trúc lưu trữ:** File dịch được lưu tách biệt dưới dạng JSON tại `src/locales/` (VD: `vi.json`, `en.json`).
3. **Quy tắc (Key Convention):** 
   - Không Code cứng (hardcode) bất kỳ chuỗi văn bản (String) nào trên thẻ UI. 
   - Sử dụng cơ chế nội suy bằng hàm `$t()`.
   - Cấu trúc khóa nên tuân theo phân tầng UI: `[TênTrang].[Khối].[TênThànhPhần]`. (Ví dụ: `$t('auth.login.btn_submit')`).
