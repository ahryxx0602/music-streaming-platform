...
1. **CSS Variables (Biến dùng chung ở Root):**
   - Áp dụng toàn bộ hệ thống mã HEX trong Design System (Primary, Surface, Text, Border...) thành các biến CSS (`--color-primary`, `--bg-dark`) bên trong file `src/assets/main.css`.
...
1. **Chủ đạo "Aurora Blue":** Sử dụng nền tối tĩnh lặng kết hợp với màu xanh công nghệ `#3B82F6`.
2. **Glassmorphism:** Sử dụng hiệu ứng mờ kính `backdrop-filter: blur(16px)` cho Navbar và Sticky Header.
...
6. **Icon System:** BẮT BUỘC sử dụng đồng nhất thư viện `@tabler/icons-vue` cho toàn bộ biểu tượng.
