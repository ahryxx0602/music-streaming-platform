# [SCR-ADMIN-01] Quản lý Mã mời Nghệ sĩ (Artist Invitations)
...
*   **Lấy dữ liệu (Fetch):**
    *   `[API-ADMIN-01]` - `GET /api/admin/invitations` (Lấy danh sách mã mời, có phân trang).
*   **Gửi dữ liệu (Mutations):**
    *   `[API-ADMIN-02]` - `POST /api/admin/invitations` (Tạo mã mời mới và gửi Email).
